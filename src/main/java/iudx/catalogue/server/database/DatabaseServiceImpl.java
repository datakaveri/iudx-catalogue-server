...
public class DatabaseServiceImpl implements DatabaseService {
  ...
  private static String INTERNAL_ERROR_RESP =
      new RespBuilder()
                                ...
                                handler.handle(
                                    Future.succeededFuture(
                                        respBuilder
                                            ...
                                            .withResult(id, UPDATE, TYPE_SUCCESS)
                                            .getJsonResponse()));
                              } else {
                                handler.handle(Future.failedFuture(INTERNAL_ERROR_RESP));
                              ...
                              handler.handle(
                                  Future.succeededFuture(
                                      respBuilder
                                          .withType(TYPE_SUCCESS)
                                          .withTitle(TITLE_SUCCESS)
                                          .withResult(id, DELETE, TYPE_SUCCESS)
                                          .getJsonResponse()));
                            } else {
                              handler.handle(Future.failedFuture(INTERNAL_ERROR_RESP));
  ...
  public DatabaseService createMlayerInstance(
    ...
    RespBuilder respBuilder = new RespBuilder();
    String mlayerInstanceID = instanceDoc.getString("mlayerInstanceID");
    String checkForExistingRecord =
        GET_MDOC_QUERY.replace("$1", mlayerInstanceID).replace("$2", "");
    client.searchAsync(
        checkForExistingRecord,
        mlayerInstanceIndex,
        res -> {
          if (res.failed()) {
            ...
            handler.handle(
                Future.failedFuture(
                    respBuilder.withType(FAILED).withResult(mlayerInstanceID).getResponse()));
              ...
              handler.handle(
                  Future.failedFuture(
                      respBuilder
                          .withType(TYPE_ALREADY_EXISTS)
                          .withTitle(TITLE_ALREADY_EXISTS)
                          .withResult(mlayerInstanceID, INSERT, FAILED, " Fail: Doc Already Exists")
                          .getResponse()));
              return;
            }
            client.docPostAsync(
                    ...
                    handler.handle(
                        Future.succeededFuture(
                            respBuilder
                                .withType(TYPE_SUCCESS)
                                .withTitle(TITLE_SUCCESS)
                                .withResultInstance(INSTANCE_CREATION_SUCCESS)
                                .getJsonResponse()));
                  } else {
