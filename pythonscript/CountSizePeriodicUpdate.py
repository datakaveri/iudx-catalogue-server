...
try:
    ...
    select_query = f"select ( SELECT count(api) FROM {databaseTableName}  ) as total_count , ( SELECT count(api) FROM {databaseTableName} where time between '{time_year_back}' AND '{utc_time}' ) as year_count , ( SELECT count(api) FROM {databaseTableName} where time between '{time_six_month_back}' AND '{utc_time}' ) as six_month_count , ( SELECT count(api) FROM {databaseTableName} where time between '{time_one_month_back}' AND '{utc_time}' ) as one_month_count , ( SELECT count(api) FROM {databaseTableName} where time between '{time_one_week_back}' AND '{utc_time}' ) as one_week_count , ( SELECT count(api) FROM {databaseTableName} where time between '{time_one_day_back}' AND '{utc_time}' ) as one_day_count "
    
    select_query2 = f"select ( SELECT COALESCE(sum(size), 0) from {databaseTableName} ) as total_size , ( SELECT COALESCE(sum(size), 0) from {databaseTableName} where time between '{time_year_back}' AND '{utc_time}' ) as year_size , ( SELECT COALESCE(sum(size), 0) from {databaseTableName} where time between '{time_six_month_back}' AND '{utc_time}' ) as six_month_size , ( SELECT COALESCE(sum(size), 0) from {databaseTableName} where time between '{time_one_month_back}' AND '{utc_time}' ) as one_month_size , ( SELECT COALESCE(sum(size), 0) from {databaseTableName} where time between '{time_one_week_back}' AND '{utc_time}' ) as one_week_size , ( SELECT COALESCE(sum(size), 0) from {databaseTableName} where time between '{time_one_day_back}' AND '{utc_time}' ) as one_day_size"
