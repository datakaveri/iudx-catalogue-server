...
pipeline {
  ...
  stage('Capture Code Coverage'){
    ...
    stage('Run Cat server for Performance Tests'){
      steps{
        script{
	  sh 'scp Jmeter/CatalogueServer.jmx ubuntu@20.193.225.59:/var/lib/jenkins/iudx/cat/Jmeter/'
          sh 'scp src/test/resources/iudx-catalogue-server.postman_collection.json ubuntu@20.193.225.59:/var/lib/jenkins/iudx/cat/Newman/'
