    ...
    stage('Run Jmeter Performance Tests'){
      steps{
        script{
          ...
          sh 'mkdir -p Jmeter ; /var/lib/jenkins/workspace/apache-jmeter-5.4.1/bin/jmeter.sh -n -t iudx-catalogue-server_complex_search_count.jmx -l JmeterTest.jtl -e -o Jmeter/'
        }
      }
    }
    // stage('Push Image') {
